...
export default function Home() {
  ...
  return (
    <main className="h-screen w-screen relative overflow-hidden bg-slate-900">
      {/* Header */}
      <header className="absolute top-0 left-0 right-0 z-[1000] bg-slate-900/90 backdrop-blur-sm px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={refreshing}
            className={`p-2 rounded-full bg-slate-800 text-white hover:bg-slate-700 transition ${refreshing ? 'animate-spin' : ''}`}
          >
            ⟳
          </button>
        </div>
      ...
      {selectedZone && zoneData && (
        <div className="absolute bottom-0 left-0 right-0 z-[1000] animate-slide-up">
          <div className="bg-slate-800 rounded-t-3xl p-6 mx-2 mb-0 shadow-2xl">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-slate-700 rounded-xl p-3 text-center">
                ...
                <p className="text-white text-2xl font-bold mt-1">{zoneData.probability}%</p>
                <p className="text-slate-400 text-xs">Probabilidad</p>
              </div>
              <div className="bg-slate-700 rounded-xl p-3 text-center">
                <span className="text-3xl">🌡️</span>
                <p className="text-white text-2xl font-bold mt-1">{zoneData.temp}°</p>
                <p className="text-slate-400 text-xs">Temperatura</p>
              </div>
              <div className="bg-slate-700 rounded-xl p-3 text-center">
                <span className="text-3xl">💨</span>
                <p className="text-white text-2xl font-bold mt-1">{zoneData.wind}</p>
      ...
      {/* Legend */}
      <div className="absolute bottom-4 left-4 z-[999] bg-slate-800/90 backdrop-blur-sm rounded-lg p-2 flex gap-2">
        <div className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-green-500"></span><span className="text-white text-xs">0-20%</span></div>
        <div className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-yellow-500"></span><span className="text-white text-xs">21-50%</span></div>
        <div className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-orange-500"></span><span className="text-white text-xs">51-80%</span></div>
        <div className="flex items-center gap-1"><span className="w-3 h-3 rounded-full bg-red-500"></span><span className="text-white text-xs">81%+</span></div>
      </div>
    </main>
  );
}
